...
class Logger:
    ...
    def _post_with_fallback(self, url: str, headers: dict, table_name: str, log_entry: dict):
        """
        Try named-parameter insert; if API complains about parameter mapping, retry positional.
        Returns (success: bool, last_error: Optional[str]).
        """
        # Try named first
        statement, parameters = self._build_named_insert(table_name, log_entry)
        payload = {"warehouse_id": self.warehouse_id, "statement": statement, "parameters": parameters}
        try:
            ...
            if "INVALID_PARAMETER_MARKER_VALUE" in emsg or "MISSING_NAME" in emsg:
                ...
                payload2 = {"warehouse_id": self.warehouse_id, "statement": stmt2, "parameters": params2}
                try:
                    ...
                    if r2.status_code in (200, 202):
                        ...
                        if state2 == "FAILED":
                            msg2 = rj.get("status", {}).get("error", {}).get("message", "Unknown error")
                            return False, msg2
                        return True, None
                    else:
    ...
    def log_event(self, type: str, level: str, message: str, source: str = "System"):
        ...
        """
        # Use exact timestamp formatting: YYYY-MM-DD HH:MM:SS
        log_entry = {
        ...
        if self.databricks_host and token and wid and self.table_name and source != "ZeroBus":
            try:
                ...
                ok, err = self._post_with_fallback(url, headers, self.table_name, log_entry)
                if not ok and err:
