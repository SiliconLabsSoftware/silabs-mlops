...
class DeviceInterface(object):
    ...
    def open(self, name: str, mode: str):
        ...
        stream = JLinkDataStream(
            name=name, mode=mode, ifc=self, stream_context=stream_context
        )
    ...
    def read(self, context: dict, length: int) -> bytes:
        ...
        data = self._jlink_read8(
            buffer_context["head"], min(length_to_end, read_length)
        )

        if read_length > length_to_end:
            data2 = self._jlink_read8(
                buffer_context["start"], read_length - length_to_end
            )
            data.extend(data2)
    ...
    def write(self, context: dict, data: bytes) -> int:
        ...
        if write_length > length_to_end:
            self._jlink_write8(
                buffer_context["start"],
                data[chunk_length : chunk_length + (write_length - length_to_end)],
            )
    ...
    def _find_context_base_address(self) -> int:
        ...
        raise Exception(
            "Timed out searching for context's base address. Ensure the device is executing properly"
        )
    ...
    def _read_context(self) -> dict:
        ...
        context["trigger_address"] = self._unpack_data(
            data, CONTEXT_OFFSET_TRIGGER_ADDR
        )
        ...
        context["command_buffer_address"] = self._unpack_data(
            data, CONTEXT_OFFSET_CMD_BUFFER_ADDR
        )
        ...
        try:
            ...
            try:
                if cmd:
                    ...
                    self._jlink.write_mem32(
                        self._context_address + CONTEXT_OFFSET_CMD_CODE * 4, cmd
                    )

                if data:
                    # Write the 'command_length' field
                    self._jlink.write_mem32(
                        self._context_address + CONTEXT_OFFSET_CMD_LENGTH * 4, len(data)
                    )
                ...
                self._jlink.write_mem32(
                    self._context_address + CONTEXT_OFFSET_STATUS * 4, status
                )
    ...
    def _issue_command(self, cmd, data=None, ignore_error=False):
        ...
        try:
            ...
            if context["command_length"] > 0:
                try:
                    response_data = self._jlink.read_mem8(
                        self._command_buffer_address, context["command_length"]
                    )
                except Exception as e:
