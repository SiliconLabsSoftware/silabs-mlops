...
class JLink(object):
    ...
    def load_binary_data(
        self, file_path: str, dest_address: int, offset=0, length=None
    ):
...
def _get_jlink_dll(library_paths):
    ...
    if curos == "win32":
        ...
        jlink, backend_info = _locate_library(
            "libjlinkarm.so", library_paths, ctypes.cdll
        )

    elif curos == "osx":
        jlink, backend_info = _locate_library(
            "libjlinkarm.dylib", library_paths, ctypes.cdll
        )
