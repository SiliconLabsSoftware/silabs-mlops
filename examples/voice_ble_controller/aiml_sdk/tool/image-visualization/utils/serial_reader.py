...
class SerialReader:
    ...
    def __init__(
        ...
        self.ignore_chars = ignore_chars or [13] + list(
            range(127, 256)
        )  # Non-ASCII and \r
    ...
    def open(self):
        ...
        self._callback_regex = [
            _CallbackRegexContext(_map_pattern(r), cb)
            for (r, cb) in as_list(self.callback_regex)
        ]
        self._captured_data = ""
        ...
        self._rx_thread = threading.Thread(
            name="SerialRx", target=self._read_loop, daemon=True
        )
    ...
    def read(
        ...
        try:
            while (time.time() - start_time) < timeout:
                ...
                if (
                    activity_timeout
                    and (time.time() - previous_activity_timestamp) > activity_timeout
                ):
                    break
