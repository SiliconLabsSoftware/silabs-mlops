...
// Accelerometer,Gyroscope and Orientation data from sensor
...
static void on_data_available(uint8_t int_id, void *ctx)
{
  ...
  sl_imu_get_gyro(g);
  sl_imu_get_orientation(o);
//  printf("%.d,%.d,%.d,%.d,%.d,%.d\r\n",m[0],m[1],m[2],g[0],g[1],g[2]);
  buffer[head_ptr].x = m[0];
  buffer[head_ptr].y = m[1];
  buffer[head_ptr].z = m[2];
  buffer[head_ptr].gx = g[0];
  buffer[head_ptr].gy = g[1];
  buffer[head_ptr].gz = g[2];
  buffer[head_ptr].ox=o[0];
  buffer[head_ptr].oy=o[1];
  buffer[head_ptr].oz=o[2];
  head_ptr += 1;
