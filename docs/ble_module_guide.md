...
The example script at [ble_receiver.py](../scripts/rpi/ble_receiver.py) uses `os.getenv("VAR", "default")` which gives you both options at once:
...
Once `ble.config()` is called, the `BLEReceiver` class handles all Bluetooth communication automatically. The example script at [ble_receiver.py](../scripts/rpi/ble_receiver.py) shows the full recommended usage:
