...
## Setup Overview
To run the Silicon Labs MLOps SDK end-to-end, users must complete the following steps:
...
*Note: You can follow the same steps to view volumes and the data stored in those volumes. Navigate to: **`<Your Catalog>`** → **`<Schema>`** → **`<Volumes>`** .*
