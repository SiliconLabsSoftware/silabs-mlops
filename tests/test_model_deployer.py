...
def test_find_remote_commander_logic(monkeypatch, deployer):
    ...
    def run_found(cmd, **k):
        return types.SimpleNamespace(
            returncode=0, stdout="/usr/bin/commander-cli\n", stderr=""
        )
...
def test_jlink_serial_detection_parsing(monkeypatch, deployer):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: types.SimpleNamespace(
            returncode=0, stdout=adapter_out, stderr=""
        ),
    )
...
def test_full_deployment_flow_orchestration(monkeypatch, deployer):
    ...
    def fake_run(cmd, **k):
        j = " ".join(cmd)
        if "which commander" in j:
            return types.SimpleNamespace(returncode=0, stdout="/bin/cmd", stderr="")
        if "adapter list" in j:
            return types.SimpleNamespace(
...
def test_interactive_selection_logic(monkeypatch, deployer):
    ...
    def fake_run(cmd, **k):
        if "adapter list" in " ".join(cmd):
            return types.SimpleNamespace(
                returncode=0, stdout="serialNumber=111\nserialNumber=222", stderr=""
            )
        return types.SimpleNamespace(
            returncode=0, stdout="Part Number : EFR32", stderr=""
        )
...
def test_error_scp_failure(monkeypatch, deployer):
    ...
    monkeypatch.setattr(
        "sml.ops.model.deployer.subprocess.run",
        lambda *a, **k: types.SimpleNamespace(
            returncode=1, stdout="", stderr="SCP fail"
        ),
    )
    with pytest.raises(RuntimeError) as exc:
...
def test_error_no_devices_connected(monkeypatch, deployer):
    ...
    monkeypatch.setattr(
        "sml.ops.model.deployer.subprocess.run",
        lambda *a, **k: types.SimpleNamespace(
            returncode=0, stdout="Empty output", stderr=""
        ),
    )
    with pytest.raises(RuntimeError) as exc:
        deployer.deploy()
...
def test_device_info_failures(monkeypatch, deployer):
    ...
    monkeypatch.setattr(
        "sml.ops.model.deployer.subprocess.run",
        lambda *a, **k: types.SimpleNamespace(
            returncode=0, stdout="No parts found here!", stderr=""
        ),
    )
    with pytest.raises(RuntimeError) as exc:
        deployer._get_device_name("u@h", "123")
