...
class TestLogger(unittest.TestCase):
    ...
    def test_logger_init(self, mock_file, mock_exists):
        ...
        logger = Logger(databricks_host="fake", client_id="id", client_secret="secret")
    ...
    def test_log_event(self, mock_post, mock_file, mock_dump, mock_load):
        ...
        self.assertEqual(
            mock_post.call_args[0][0], "http://host/api/2.0/sql/statements"
        )
    ...
    def test_get_token_failure(self, mock_post):
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_any_call(
                "Warning: Failed to fetch Databricks OAuth token: Auth failed"
            )
    ...
    def test_resolve_warehouse_id_failure(self, mock_get):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "warehouses": [{"name": "Different", "id": "w123"}]
        }
        mock_get.return_value = mock_response
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_called_with(
                "Warning: Could not find warehouse named 'MyWarehouse'"
            )
        ...
        with patch("builtins.print") as mock_print:
            ...
            self.assertIsNone(wid)
            mock_print.assert_called_with(
                "Warning: Failed to resolve warehouse ID: API Down"
            )
    ...
    def test_log_event_api_failure(self, mock_post, mock_file, mock_load, mock_dump):
        ...
        with patch("builtins.print") as mock_print:
            logger.log_event("Profiling", "Info", "Started")
            mock_print.assert_called_with(
                "Warning: Failed to stream log to Databricks: SQL API Error"
            )
