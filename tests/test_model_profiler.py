...
from silabs_mlops.model.profiler import NPUProfiler, ProfileResult, DeviceInfo
...
class TestNPUProfiler(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher = patch('silabs_mlops.logs.Logger')
    ...
    def test_resolve_profiler_via_sdm_relative(self, mock_which):
        ...
        mock_which.side_effect = lambda name: "/opt/silabs/bin/sdm" if name == "sdm" else None
    ...
    def test_resolve_profiler_not_found(self, mock_run, mock_which):
        ...
        self.assertIn("Silicon Labs NPU Toolkit", str(context.exception))

    @patch('subprocess.run')
    @patch('silabs_mlops.model.profiler.NPUProfiler._resolve_sdm')
...
"""
    ...
    @patch('subprocess.run')
    @patch('silabs_mlops.model.profiler.NPUProfiler._resolve_profiler')
    @patch('silabs_mlops.model.profiler.NPUProfiler._collect_results')
    @patch('silabs_mlops.logs.Logger.log_model_profiling')
    def test_profile_gui(self, mock_log, mock_collect, mock_resolve, mock_run, mock_exists):
    ...
    def test_profile_standard(self, mock_file, mock_collect, mock_popen, mock_resolve, mock_exists):
        # Model path checking
    ...
    def test_parse_layers_extraction(self):
        ...
        mock_layers_raw = [
            {"name": "conv2d", "input_shape": "[1, 28, 28, 1]", "output_shape": "[1, 28, 28, 16]"},
    ...
    @patch('subprocess.Popen')
    @patch('builtins.open', new_callable=mock_open)
    def test_profile_subprocess_fail(self, mock_file, mock_popen, mock_resolve, mock_exists):
        mock_exists.return_value = True
        mock_resolve.return_value = ["mvp_profiler"]
    ...
    @patch('subprocess.Popen')
    @patch('builtins.open', new_callable=mock_open)
    def test_profile_subprocess_timeout(self, mock_file, mock_popen, mock_resolve, mock_exists):
        mock_exists.return_value = True
        mock_resolve.return_value = ["mvp_profiler"]
        
        mock_proc_instance = MagicMock()
        mock_proc_instance.stdout = []
        mock_proc_instance.wait.side_effect = subprocess.TimeoutExpired(cmd="mvp_profiler", timeout=600)
        mock_popen.return_value = mock_proc_instance
    ...
    def test_parse_summary_txt(self):
        ...
        mock_summary_file.read_text.return_value = "Arena size : 158.5 KB\nTotal MACs | 3.5 M\nBoard      : BRD2601B\n"
        
        result = ProfileResult(model_name="test", model_path="test", device_id="1", output_dir="o")
    ...
    @patch('silabs_mlops.model.profiler.requests.post')
    @patch('silabs_mlops.model.profiler.requests.put')
    @patch('os.walk')
    @patch('builtins.open', new_callable=mock_open)
    def test_upload_to_volume(self, mock_file, mock_walk, mock_put, mock_post):
        ...
        res = self.profiler._upload_to_volume(Path("/tmp/fake_dir"), "model", "/Volumes/main/default/vol1")
    ...
    @patch('silabs_mlops.model.profiler.NPUProfiler._collect_results')
    @patch('builtins.open', new_callable=mock_open)
    def test_profile_use_simulator(self, mock_file, mock_collect, mock_popen, mock_resolve, mock_exists):
        # Simulator avoids setting `--device`
        mock_exists.return_value = True
        mock_resolve.return_value = ["mvp_profiler"]
        mock_collect.return_value = ProfileResult(model_name="sim", model_path="sim", device_id="", output_dir=".")
    ...
    def test_print_summary(self, mock_print):
        ...
        result = ProfileResult(
            model_name="x", model_path="x", device_id="x", output_dir="o",
            board="B1", arena_size_kb=10.0, total_macs=100, summary_txt_path="s.txt"
        )
